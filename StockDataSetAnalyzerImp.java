...
public class StockDataSetAnalyzerImp implements StockDataSetAnalyzer {
        ...
        StockHistoryDataSet SHDA ;
        
        public StockDataSetAnalyzerImp()
        {
            SHDA = new StockHistoryDataSetImp();
        }
            ...
            if (! (startDate == null && endDate == null) && (! companies_name.empty()))
            {
                ...
                for ( int i=0; i< companies_name.size() ; i++)
                {
                   ...
                   String CompanyName = companies_name.retrieve();
                    StockData data1 =  SHDA.getStockHistory(CompanyName).getStockData(startDate);
                    StockData data2 =  SHDA.getStockHistory(CompanyName).getStockData(endDate);
...
*/
    ...
    public static void main(String[] args) {
        ...
        System.out.println("************* getSortedByPerformance");
        DLLComp<CompPair<String, Double>> per = Analyzer.getSortedByPerformance(new Date("1/30/1971"), new Date("6/4/1971"));
        if (! per.empty())
        {
            per.findFirst();
        ...
        System.out.println("************* getSortedByVolume");
        DLLComp<CompPair<String, Long>>  vol = Analyzer.getSortedByVolume(new Date("1/30/1971"), new Date("6/4/1971"));
        if (! vol.empty())
        {
            vol.findFirst();
        ...
        System.out.println("************* getSortedByMSDPI");
        DLLComp<CompPair<Pair<String, Date>, Double>> SDPI = Analyzer.getSortedByMSDPI(new Date("1/30/1971"), new Date("6/4/1971"));
        if (! SDPI.empty())
        {
            SDPI.findFirst();
